...
Investigate standalone and utility skills not covered by the process or init suites: `github-issue`, `rfc`, `post-meeting`, `agent-evaluation`, `writing-skills`, `using-skills`, `icon-init`, `icon-status`, `start-worktree`, `dependency-management`, `code-quality-rules`, `context-document-guidelines`, `manager-routing-guide`.
