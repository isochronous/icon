...
- Without a stated budget, the audit cycle is the only friction signal, and the cycle fires on retrospective discovery rather than at the moment of growth. A budget makes the question "are we within ceiling?" answerable on every MR, not only at audit time.
...
- **Re-audit trigger**:
  ...
  > Any merge request that grows the manager session total or the PM session total by **≥ 5% of the budget cap** (≥ 425 words for manager, ≥ 350 words for PM) must re-run the word-count inventory before merge and update both ADR-008 and the snapshot artifact (`.context/tasks/<latest>/word-count-snapshot.md` pattern) with the new numbers.

  - **Cumulative-drift re-inventory trigger (audit-scoped)**: independent of the per-MR trigger above, a re-inventory is **mandatory** whenever an audit measures any single always-loaded component to have grown **≥ 5% of its session budget above its baseline** (≥ 425 words for the manager, ≥ 350 words for PM), regardless of whether any individual MR ever tripped the per-MR threshold. The per-MR trigger is MR-scoped and provably misses slow drift: the manager grew **+978 words** cumulatively (4,148 → 5,126) without any single MR's net delta reaching 425 words. The two triggers are complementary — the per-MR gate catches fast growth at merge time; the cumulative-drift gate catches slow accretion at audit time. When this gate fires, follow the same re-inventory + ADR-update procedure defined below, and additionally decide per the per-component cap whether to reduce the component or record an explicit accept-with-rationale overage (see Consequences).
  ...
  - The trigger is **MR-scoped**, not commit-scoped: a single MR's net delta is what matters. Internal commits within an MR that grow-then-trim are fine.
...
- The snapshot artifact gives reviewers a concrete number to compare an incoming MR against, with no extra tooling.
...
5. **Trigger on every MR (no threshold)** — rejected. Re-inventorying on every documentation typo fix is process friction with no signal. 5% is the smallest threshold that fires only when growth is structural.
...
- GitLab issue #18 (O-T1 audit, O-T2 / O-T3 / O-T4 trims shipped under the same MR).
