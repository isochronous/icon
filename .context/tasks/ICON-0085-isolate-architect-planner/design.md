...
> **CORRECTION (post-review, 2026-07-18):** This section's premise — that an isolated sub-agent "cannot clarify live" and therefore *must* replace clarification with report-borne open questions — is **FALSE**. A sub-agent CAN use `ask_user` (ICON's portable name for the harness user-input tool); isolation is a separate context window + a returned report, not loss of the user-input channel. As shipped, `@planner`/`@architect` **retain `ask_user`** for user-facing clarifications (the shared common-constraints rule applies to them), and the report `### Open Questions & Assumptions` section is a **complement** (documents assumptions made + surfaces questions the *manager* should resolve) — not a replacement. What genuinely goes away is only the *inline mid-run dialogue with the manager* (an isolated agent returns a report instead). OQ-2 and the "documented per-agent precedence" below are moot — there is no `ask_user`-vs-report tension; both channels coexist. The agent defs and ADR-015 reflect the corrected framing.
