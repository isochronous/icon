...
A skill is a named, reusable process instruction. When invoked, the tool reads `skills/<name>/SKILL.md` and the active agent follows it. Skills encode proven workflows that would otherwise be improvised inconsistently — the plugin's mechanism for making good process repeatable.
...
**Critical:** The tool maps `/skill-name` to the **directory name**, not the `name:` field in frontmatter. If the directory name and frontmatter `name:` diverge, invocation fails silently.

## Skill Types

The canonical skill registry is `ls skills/` (shipped) and `ls .claude/skills/` (maintainer-only). The tables below give representative examples per category — not exhaustive, and may lag the directory listing.

### Rigid Discipline Skills
Process gates that enforce non-negotiable behavior — specific phases, checklists, or required outputs.
...
Guidance for open-ended work requiring judgment. Less prescriptive than discipline skills.
...
| Common-constraints block | A shared text block (`shared/common-constraints.md`) defining core rules for all agents: communication, codebase respect, verification, self-review, anti-rationalization, scope discipline, task artifacts | **Not a skill.** Inlined verbatim into each `agents/*.agent.md` between `<!-- BEGIN: common-constraints -->` and `<!-- END: common-constraints -->` sync markers, so it is always-loaded with the agent definition — no invocation step. Authoritative source is `shared/common-constraints.md`; agent copies must remain byte-equal to it. |

**Enforcement chain**: The injected common-constraints block instructs every agent to follow the listed rules from the first token — including the skill-awareness rule. The `using-skills` skill, invoked by each dispatcher at session start (reachable from the constraints' "skill awareness" expectation), then mandates applicable skills be invoked before any other action. The result is a self-reinforcing loop that cannot be bypassed by skipping either layer.

**Sync mechanism**: A pre-commit hook at `.githooks/pre-commit` enforces byte-equality of the inlined block across all nine agents. It reads `shared/common-constraints.md` as the authoritative source and, for each `agents/*.agent.md`, rewrites the content between the `<!-- BEGIN: common-constraints -->` and `<!-- END: common-constraints -->` markers to match. Marker detection is fenced-code-block aware (CommonMark §4.5), so agent files may safely illustrate the BEGIN/END marker strings inside backtick or tilde fenced code blocks — those are treated as literal text, not sync markers. Any rewritten file is re-staged so the in-flight commit includes the synced content. The hook aborts on structural errors (missing source file, or an orphaned marker — one of the BEGIN/END pair without the other), and skips agent files carrying no markers. It is wired via `git config core.hooksPath .githooks`, the same mechanism as the `post-commit` cache-pruning hook.

**Authoring rule — scope before delegation**: Before delegating any edit that adds a new behavioral rule, decide whether it applies to one agent or all. If all agents, the rule belongs in `shared/common-constraints.md` (propagated by the hook above) — not in a single agent's body. Adding a universal discipline to one agent's file and later relocating it requires a kill-and-redispatch; deciding scope first makes the delegation correct on the first dispatch.
...
`@context-specialist` is backed by internal skills (`context-specialist-create`, `context-specialist-detect-tree-position`, `context-specialist-impl-leaf`, `-branch`, `-root`, `context-document-guidelines`). Not directly invocable.
...
Live in `.claude/skills/` rather than the shipped `skills/` tree; not distributed to end users.
...
**When the task is "edit skill X", invoke `using-skills` AND the target skill itself BEFORE the first `Read` or `Edit` on its `SKILL.md`.** The discipline of the skill being edited governs the edit — not only the discipline of skills consumed for unrelated work. This applies even to edits that feel "small" or "obvious" (adding a section, fixing a reference): a skill's own iron law (e.g. `writing-skills`' "edit skill without testing? same violation") fires regardless of size. Re-invoke the target skill even if you read it recently — its specifics may have evolved, and re-reading is cheap. The rationalization to watch for is "this addition is too small for the workflow"; that thought is the trigger to stop and invoke `using-skills`.
...
- **Frontmatter invocability semantics** — `user-invocable: false` and `disable-model-invocation: true` are independent gates with different targets:
  - `user-invocable: false` — hides the skill from the user slash-command menu; no effect on whether an agent or dispatcher can invoke it.
...
`<SCREAMING_SNAKE_CASE>` slots in a dispatcher prompt are **unfilled placeholders** — values the orchestrator substitutes before dispatching the agent. They are never present in the prompt the dispatched skill actually reads.
...
- See also: [skill decomposition](../standards/skill-decomposition.md) — wiring, structural, and sweep conventions for individual skills
