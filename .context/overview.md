...
Pure content. Markdown for definitions, JSON for the manifest and hook wiring, two cross-platform Node.js (`.mjs`) hook wrappers — SessionStart and PreToolUse (see `.context/domains/hooks.md`) — bash for the `.githooks/` gates and the maintainer-only scripts under `.claude/skills/*/scripts/`, and bash + PowerShell for the plugin-shipped helpers under `skills/*/scripts/`, where `context-graph` and `append-retrospective-entry` ship as `.sh`/`.ps1` parity pairs because they run in the consumer's environment (see `.context/standards/shell-portability.md`). No compile step, no test runner, no package manager (ADR-005) — but committed, dependency-free scripts run in place ARE in scope. Validation is the `.githooks/pre-commit` gate set, the `security` CI workflow (gitleaks / semgrep / shellcheck), and structural review during PR. The plugin bundles no MCP servers — GitHub access is via the `gh` CLI (see `.context/domains/github-access.md`).
