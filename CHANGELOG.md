...
- Added session-per-phase task execution: `plan.md` now carries a `## Phase State` pointer and an append-only `## Phase Handoff Log`, every task-plan phase reconstructs from the committed `plan.md` fail-closed on entry (instead of relying on session memory), the manager gains a `{task_id, task_folder, phase}` entrypoint, and a new `generate-phase-launcher` skill emits a harness-specific launcher (`target-harness` ∈ claude-code/copilot-cli/generic) that runs each phase in a fresh, fail-closed, injection-safe session for headless/cron/CI execution; the plan.md hardening ships in `context_template/` (schema 1.9→1.10) so `/upgrade-repo` applies it to existing repos. (ICON-0082)
- Added model-aware delegation: the manager and product-manager now select a model tier for every isolated delegation — Haiku for basic/mechanical work, Sonnet by default, Opus for complex/architectural/security/ambiguous tasks — cutting cost on cheap work and raising quality on hard work; the full task→tier mapping lives in the `manager-routing-guide` skill, expressed portably via tier aliases (advisory where a harness lacks per-subagent model control). (ICON-0084)
...
- Tersened agent roles, skills, commands, and `.context`/template docs (~4,800 words, behavior-preserving) to reduce always-loaded token weight — the manager role is ~13% lighter; template docs ship via schema 1.10→1.11. (ICON-0083)
- The manager now dispatches `@architect` and `@planner` as isolated task→report sub-agents (self-contained warmstart in, structured report out) like the other specialists, instead of sharing its session context with them — they get clean scoped context, run under their own model tier (Opus for architecture, Sonnet for planning), and can run in parallel; they record assumptions and manager-facing scope questions in a report `Open Questions & Assumptions` section (and still use `ask_user` for user-facing clarifications). The warmstart template gains an `### Architecture` field so a cold specialist has the design context it needs. (ICON-0085)
