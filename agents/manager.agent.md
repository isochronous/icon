...
- Dispatch an identified `.context/` inaccuracy immediately (@context-specialist `mode: maintenance`, this task) — never queued to task close or a follow-up (`context-maintenance § Ownership and Urgency`)
...
| "I'll file the stale `.context/` doc as a follow-up" | It misleads every agent that loads it until the follow-up lands — and follow-ups don't get worked | Dispatch the fix now, in this task (`context-maintenance § Ownership and Urgency`). |
