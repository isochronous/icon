...
## initialize-multimodule: Step 9: Push and open pull requests

Push and PR creation is **per git repo** — one PR per unique git root. Iterate
...
- PR URLs (one per unique git repo) from Step 9
- Sub-projects skipped from push + PR because they are not inside any git repo
