...
Create mode walks a user through scaffolding a new Claude Code plugin from an empty directory to a working, optionally marketplace-ready, ICON-initialized plugin. Each phase is loaded on demand from a companion file in this directory.

## Precondition

Confirm a working directory exists for the new plugin and that the current shell is operating inside it:
...
If the directory does not exist, offer to create it (`mkdir -p <plugin-name> && cd <plugin-name>`) before continuing. If the directory contains files that look like an existing project (e.g., a `plugin.json` is already present), pause and confirm with the user whether this is a resume or a re-init.
...
Record the answer. A `y` response runs all 5 phases; `n` (or empty) runs only Phases 1–4 and skips `create-phase-marketplace.md`.

## Phase Sequence

Load and execute each phase file in order. A returning user may already have completed earlier phases — confirm where to resume rather than re-running phases that have already produced outputs.
