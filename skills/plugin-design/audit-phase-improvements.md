...
This phase is heuristic, not prescriptive — surface the opportunity and let the user decide whether to act.

## Heuristics

Walk the following list and produce at least one suggestion per category that applies to the plugin. If a category does not apply (e.g., no `hooks/` directory), skip it.
...
2. **Token economy on always-loaded surfaces** — measure the byte size of every `agents/*.agent.md` and any `shared/common-constraints.md`-equivalent file. Files that load on every agent invocation should stay small (rough target: < 8 KB each). Suggest extractions when one is bloated.
...
4. **Retrospective-wisdom automation** — is there a mechanism (a skill, a script, or a documented manual step) for promoting recurring retrospective lessons to standards? Without one, retros become write-only and the same lessons repeat.
