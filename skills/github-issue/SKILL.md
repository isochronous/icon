...
name: github-issue
description: >
  Renders provided story content into a standard GitHub issue (Markdown issue body) with title, description, acceptance criteria as a task list, labels, and cross-references. The rendering logic is generic; examples are neutral.
user-invocable: true
---

# GitHub Issue Writer Skill

## Overview

Render the provided story content into a standard GitHub issue body in Markdown.

## Inputs

**Required:**
- Story content (can be rough ideas, research findings, or existing issue text to refine)
...
- `output_filename`: File name for the issue (e.g., `42-filter-panel-autocomplete.md`)
...
- Labels / area preference (e.g., `frontend`, `backend`, `spike`)
- Existing issue for refinement
...
A cleanly formatted GitHub issue body with:
...
Follow this format for all issues. Use GitHub-flavored Markdown:

```
## [Area] Summary of Story

### Objective
[1-3 sentences. Clear and concise statement of what this issue accomplishes. Professional language for all stakeholders.]
...
[List conditions that must be met for completion from a QA/testing perspective, rendered as a GitHub task list (`- [ ]`). Each AC should be:
...
Format each item as a checkable Given-When-Then for clarity:]
...
**Filter out:**
* ❌ Framework features that are obvious for the project's stack
...
* ✅ Patterns to follow: "Use same autocomplete pattern as the transfers modal (lines 74-86)"
* ✅ Non-standard dependencies: "Requires upgrade to the UI client library v2.3.0+"
* ✅ Migration notes: "Replaces legacy dropdown logic in `RequestService.getMakes()`"
* ✅ Configuration impacts: "Blocked until CHECK_VALID_MAKE feature flag enabled in QA environment"
...
* Patterns: "Follow same error handling as the payments module"

**Dependencies section rules:**
* Cross-reference parent or blocking issues with `#123` (or `owner/repo#123` for another repo); don't restate their content
* Only list external dependencies if they require action (e.g., version upgrade, new package install)
* Omit standard project dependencies
...
### Labels
`area-label`, `type-label` [— suggested labels; apply with `gh issue create --label`]

### References
- Blocked by #123
- Related: owner/repo#456
```
...
- Task list (checkbox): `- [ ] item` (unchecked) / `- [x] item` (checked)
- Cross-reference an issue or PR: `#123`, or `owner/repo#123` across repositories
- Auto-close on merge: closing keywords in a PR body (`Closes #123`, `Fixes #123`)
...
**Technical Notes:**
Dependencies:
* Parent issue: #1042 (Backend API must return FilterOptionDTO[] with both code and label fields)
* Client library: @myorg/api-client package must have updated filter endpoints returning FilterOptionDTO[]
* UI library: DatePicker component (already available in the project)
