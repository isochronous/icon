...
description: >
  ...
  generates root-level cross-project context and raises a merge request for
...
without a human reviewing and merging a merge request.
...
## initialize-monorepo: Step 6: Push Branch and Open Merge Request
...
If possible with available tools, then create a merge request targeting `$INTEGRATION_BRANCH`:

- **Title**: `chore: initialize agent-system context`
- **Description**:
  ```
  This MR was generated by the initialize-monorepo skill.
...
**Do not merge this MR yourself.** Surface the MR URL to the user and ask
...
| Merging the MR without human review | Always surface the MR URL and stop — do not self-merge |
