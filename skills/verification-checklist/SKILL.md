...
Before claiming a task complete, you must have **command output** proving it works. Internal reasoning is not evidence.

| Claim | Required Evidence |
|-------|-------------------|
| "Tests pass" | Test runner output with pass count |
| "Build succeeds" | Build command output with success status |
| "Lint is clean" | Linter output with no errors |
| "Feature works" | The command or test that exercises the feature |
| "Bug is fixed" | The previously-failing test now passing |
...
| "Build succeeded — that means tests pass" | Build is not test. Tests are not lint. Each needs its own command output. |
...
Before marking a task complete, pass these gates in order. A failure at any gate means the task is NOT complete.
...
The implementation matches the task spec — no more, no less. Check for:
- **Under-delivery**: Specified requirements not implemented
- **Over-delivery**: Unrequested changes (scope creep, "while I was here" changes, preemptive error handling for unobserved cases)
- **Drift**: Implementation addressing a different problem than specified
...
Review your completion report for rationalization language:
- "Should" → "does" backed by evidence
- "Would" → "did" backed by evidence
- "Probably" → certainty, or acknowledge the gap
- "Minor" / "trivial" / "simple" → these minimize risk. Remove them and evaluate objectively.
...
- About to mark a task complete without command output in your response.
- Tests written but the test command not executed.
- Build succeeded and you're about to claim "all checks pass" without running tests or lint.
...
3. Walk the Completion Quality Gate above, failing any gate where evidence is missing.
