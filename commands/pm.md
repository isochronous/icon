...
Switch your operating role to the ICON **product-manager** agent. Maintain this role for every subsequent turn in this session until the user runs `/ICON:manager` to switch back, explicitly invokes a different agent, or ends the session.
...
2. Read the file. Treat its entire contents — purpose, scope, workflow, constraints — as your active role definition. The role replaces whatever role you were previously operating under (typically `@manager`).
